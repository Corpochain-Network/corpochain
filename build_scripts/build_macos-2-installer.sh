...
DMG_NAME="corpochain-${CORPOCHAIN_INSTALLER_VERSION}.dmg"
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/corpochain-"${CORPOCHAIN_INSTALLER_VERSION}"-arm64.dmg
...
if [ "$NOTARIZE" == true ]; then
  ...
  notarize-cli --file="$DMG_NAME" --bundle-id pl.corpochainblockchain.blockchain \
...
# xcrun altool --notarize-app -f Corpochain-0.1.X.dmg --primary-bundle-id pl.corpochainblockchain.blockchain -u username -p password
...
# xcrun stapler staple Corpochain-0.1.X.dmg
#
# Validate DMG:
# xcrun stapler validate Corpochain-0.1.X.dmg
