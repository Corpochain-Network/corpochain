...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        all_targets = []
        stop_searching_for_farmer = "kop_target_address" not in config["farmer"]
        stop_searching_for_pool = "kop_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("kop_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("kop_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("kop_target_address") or all_targets[-2] == config["pool"].get(
                    "kop_target_address"
                ):
        ...
        if "kop_target_address" not in config["farmer"]:
            print(
                f"Setting the kop destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["kop_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["kop_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['kop_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "kop_target_address" not in config["pool"]:
            print(f"Setting the kop destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["kop_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["kop_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['kop_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the KOP destination addresses, edit the `kop_target_address` entries in"
...
def chia_init(
    ...
    *,
    ...
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CRYPTOMINES_ROOT is set, or if user has run corpochain init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
