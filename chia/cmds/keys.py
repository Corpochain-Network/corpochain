...
def keys_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with corpochain init")
...
@click.option(
    ...
    "-d",
    help=(
        "Show the first wallet address using non-observer derivation. Older Corpochain versions use "
...
@click.option("--prefix", "-x", help="Address prefix (cch for mainnet, tcch for testnet)", default=None, type=str)
@click.pass_context
def search_cmd(
    ctx: click.Context,
