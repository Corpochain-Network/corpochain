...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    os.environ["CORPOCHAIN_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your CORPOCHAIN_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['chia']} TCCH."
        )
        if show_addresses:
            print("All Addresses: ")
