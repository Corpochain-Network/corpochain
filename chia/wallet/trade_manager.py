...
class TradeManager:
    ...
    By default, standard CCH is supported but to support other types of assets you must implement certain functions on
    ...
    async def cancel_pending_offers(
        ...
        for trade_id in trades:
            ...
            for coin in Offer.from_bytes(trade.offer).get_cancellation_coins():
                ...
                # This should probably not switch on whether or not we're spending a CCH but it has to for now
    ...
    async def _create_offer_for_ids(
        ...
        try:
            ...
            for id, amount in offer_dict.items():
                ...
                # asset_id can either be none if asset is CCH or
                ...
                if amount > 0:
                    ...
                    # For the CCH wallet also include the fee amount to the coins we use to pay this offer
                ...
                if asset_id is not None and wallet is not None:  # if this asset is not CCH
            ...
            # The access of the sorted keys here makes sure we create the CCH transaction first to make sure we pay fee
            # with the CCH side of the offer and don't create an extra fee transaction in other wallets.
            for id in sorted(coins_to_offer.keys()):
                ...
                # This should probably not switch on whether or not we're spending CCH but it has to for now
                if wallet.type() == WalletType.STANDARD_WALLET:
