...
install_python3_and_sqlite3_from_source_with_yum() {
  ...
  cd "$TMP_PATH"
  # Install sqlite>=3.37
  # yum install sqlite-devel brings sqlite3.7 which is not compatible with corpochain
  ...
  # yum install python3 brings Python3.6 which is not supported by corpochain
  cd ..
...
if [ "$(uname -m)" = "armv7l" ]; then
  echo ""
  echo "WARNING:"
  echo "The Corpochain Blockchain requires a 64 bit OS and this is 32 bit armv7l"
  echo "Exiting."
  exit 1
fi
...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "12" ]; then
  echo "Corpochain requires Python version >= 3.7 and  < 3.12.0" >&2
...
echo ""
echo "Corpochain blockchain install.sh complete."
...
echo ""
echo "Type '. ./activate' and then 'corpochain init' to begin."
